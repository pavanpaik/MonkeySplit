...
| ORM | **Drizzle ORM** |
| Auth | **NextAuth.js v5 (Auth.js)** — Google, GitHub, Email magic-link providers |
| Real-time | **Pusher** or **Ably** (live balance/activity updates) |
...
- `expenses(group_id, deleted_at)` — group expense listings
- `expenses(friendship_id)` — friend expense lookups
- `expense_participants(user_id)` — balance calculations
- `activity_log(group_id, created_at)` — activity feed queries
- `notifications(user_id, read)` — unread notification counts

### Schema Evolution

- Generate migrations with `npx drizzle-kit generate:pg` after schema changes
- Apply migrations: `npx drizzle-kit push:pg` (development) or `npx drizzle-kit migrate` (production)
...
- Prefer additive migrations (nullable columns, new tables) for zero-downtime deploys
...
### 4.2 Debt Simplification (Greedy)
- Input: Map of user → net balance
- Output: Minimal set of transfers

### 4.3 Multi-Payer Split Resolution
...
### 5.3 Group Deletion with Unsettled Balances
...
│   ├── admin/export/route.ts       — GDPR data export
│   └── cron/
│       ├── recurring-expenses/route.ts — Vercel Cron handler
│       └── purge-deleted/route.ts      — Purge soft-deleted records past retention
├── terms/page.tsx                  — Terms of Service (static MDX)
├── privacy/page.tsx                — Privacy Policy (static MDX)
└── globals.css
```
...
- CSRF tokens for all state-changing operations (Auth.js provides this by default)
- Secure cookie settings: `httpOnly`, `secure`, `sameSite: 'lax'`

### Rate Limiting

Apply rate limiting in Next.js middleware (`middleware.ts`) using **Upstash Redis** with the `@upstash/ratelimit` sliding window algorithm. Return `429 Too Many Requests` with `Retry-After` header.
...
- Use React Suspense + streaming for data-heavy pages
- Mobile-first responsive design: bottom tab navigation on mobile, sidebar on desktop

### Database Optimization
- Add indexes listed in section 2
- Use `EXPLAIN ANALYZE` on critical queries during development; aim for index scans over sequential scans
...
### Deployment Strategy
- **Preview deployments**: Vercel auto-deploys every PR to a unique preview URL — require passing CI checks before merge
...
### Health Check
- `GET /api/health` → returns `{ status: 'ok', db: 'connected', timestamp }` by querying the database with `SELECT 1`
...
- Soft-deleted expenses: permanently purged after 90 days via a scheduled cron job (`/api/cron/purge-deleted`)
